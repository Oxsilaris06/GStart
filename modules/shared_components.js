            ...
            return `
                <div class="shared-module-form" id="form_adversaire_${containerId}">
                    ...
                    <div class="form-row-dynamic" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
                        <div><label>Nom / Surnom / Signalement</label><input type="text" id="adv_nom_${containerId}" placeholder="Ex: Individu armé, 'Le balafré'..."></div>
                        <div><label>Morphologie</label><input type="text" id="adv_morpho_${containerId}" placeholder="Taille, corpulence..."></div>
                        <div><label>Tenue vestimentaire</label><input type="text" id="adv_tenue_${containerId}" placeholder="Veste noire, jean..."></div>
                        <div><label>Armement / Dangerosité</label><input type="text" id="adv_arme_${containerId}" placeholder="Couteau, arme de poing..."></div>
                    </div>
                    ...
                    <div style="background: rgba(220, 38, 38, 0.05); border: 1px dashed var(--danger-red, #dc2626); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px;">
                            <div><label>TAJ</label><input type="text" id="adv_taj_${containerId}" placeholder="Positif / Négatif / Actes"></div>
                            <div><label>FPR</label><input type="text" id="adv_fpr_${containerId}" placeholder="Fiche S, etc..."></div>
                            <div><label>SIA / FINIADA</label><input type="text" id="adv_sia_${containerId}" placeholder="Détenteur ?"></div>
                            <div><label>SIV / FOVES</label><input type="text" id="adv_siv_${containerId}" placeholder="Véhicule connu ?"></div>
                        </div>
                    </div>
    ...
    Pax: {
        renderForm(type, containerId, onSaveCallbackName) {
            ...
            if (type === 'intervenants') {
                colorSelect = `
                    <div>
                        <label>Couleur (Affichage Timeline)</label>
            ...
            return `
                <div class="shared-module-form" id="form_pax_${containerId}">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 15px;">
                        <div>
                            <label>Nom / Prénom / Indicatif</label>
                            <input type="text" id="pax_nom_${containerId}">
                        </div>
                        ${type !== 'intervenants' ? `
                            <div>
                                <label>${type === 'amis' ? 'Unité / Fonction' : 'Âge / Genre'}</label>
                                <input type="text" id="pax_desc1_${containerId}">
                            </div>
                            <div>
                                <label>${type === 'amis' ? 'Mission / TPH' : 'État / Blessure potentielle'}</label>
