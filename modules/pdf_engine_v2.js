...
const PDFEngineV2 = {
    ...
    async collectAllData() {
        console.log("📸 Début collecte des photos Base64...");
                ...
                formData.dynamic_photos[category].forEach(photoMeta => {
                    promises.push((async () => {
                        try {
                            ...
                            if (blob) {
                                photosBase64[photoMeta.id] = await this.blobToBase64(blob);
                                console.log(`✓ Photo chargée: ${photoMeta.id} (${category})`);
                            } else {
                                console.warn(`⚠ Photo non trouvée dans DB: ${photoMeta.id}`);
                            }
                        } catch (e) { console.error(`✗ Erreur chargement photo ${photoMeta.id}`, e); }
                    })());
                });
            }
            await Promise.all(promises);
        }
        console.log(`📸 Fin collecte. ${Object.keys(photosBase64).length} photos récupérées.`);
        return {
    ...
    generateHTML(data, isPreview = false) {
        ...
        const css = `
            <style>
                ...
                * { box-sizing: border-box; -webkit-print-color-adjust: exact; }
                ...
                th, td { border: 1px solid ${colors.border}; padding: 8px; text-align: left; word-wrap: break-word; overflow: hidden; }
            ...
            <div class="pdf-page">
                ...
                <div class="pdf-header">
                    ...
                    <div class="header-info">
                        <div>OPÉRATION DU : <strong>${formData.date_op || '-'}</strong></div>
                        <div>RÉDACTEUR : <strong>${formData.trigramme_redacteur || '-'}</strong></div>
                        <div>UNITÉ : <strong>${formData.unite_redacteur || '-'}</strong></div>
                    </div>
                </div>
                ...
                <div class="grid">
                    <div class="card">
                        ...
                        <div class="label">Situation Générale</div><div class="value">${formData.situation_generale || '-'}</div>
                        <div class="label">Situation Particulière</div><div class="value">${formData.situation_particuliere || '-'}</div>
                    </div>
                    <div class="card">
                        <h3>CIBLES(S)</h3>
                        ...
                        `).join('') : '<div class="value">Aucune cible renseignée.</div>'}
                    </div>
                </div>
            </div>
        `;
            ...
            pages += `
                <div class="pdf-page">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        ${formData.adversaries.slice(0, 3).map((adv, idx) => {
                                        ...
                                        <div style="flex: 1;">
                                            ...
                                            <div style="display: grid; grid-template-columns: 1fr 1.5fr; gap: 15px; font-size: 0.85em;">
                                                <div>
                                                    ...
                                                    <div>Born: ${adv.date_naissance || '-'} @ ${adv.lieu_naissance || '-'}</div>
                                                    <div>Job: ${adv.profession_adversaire || '-'}</div>
                                                    <div>Famille: ${adv.situation_familiale || '-'}</div>
                                                    <div style="margin-top:5px;"><span class="label">Esprit</span> ${(adv.etat_esprit_list || []).join(', ') || '-'}</div>
                                                </div>
                                                <div>
                                                    <span class="label">Localisation & Moyens</span>
                                                    <div><strong>Domicile:</strong> ${adv.domicile_adversaire || '-'}</div>
                                                    <div style="margin-top:5px;"><strong>Véhicules:</strong> ${(adv.vehicules_list || []).join(' | ') || '-'}</div>
                                                    <div style="margin-top:5px;"><strong>Moyens (ME):</strong> ${(adv.me_list || []).join(', ') || '-'}</div>
                                                </div>
                                            </div>
                                            ...
                                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 10px; padding-top: 10px; border-top: 1px dashed ${colors.border}; font-size: 0.85em;">
                                                ...
                                                <div>
                                                    ...
                                                    <div><strong>Armes:</strong> ${adv.armes_connues || '-'}</div>
                                                    <div><strong>ATCD / TAJ:</strong> ${adv.antecedents_adversaire || '-'}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="pdf-page">
                ...
                <div class="grid">
                    <div class="card"><h3>Chronologie Prévisionnelle</h3><table><thead><tr><th style="width:80px;">Heure</th><th>Événement</th></tr></thead><tbody>
                        ${(formData.time_events || []).length > 0 ? formData.time_events.map(ev => `<tr><td class="monospaced">${ev.hour || ''}</td><td>${ev.type || ''}${ev.description ? ' : ' + ev.description : ''}</td></tr>`).join('') : '<tr><td colspan="2">N/A</td></tr>'}
                    </tbody></table></div>
                    <div class="card"><h3>Hypothèses d'ensemble</h3>
                        <div class="label">H1 (Succès)</div><div class="value">${(formData.hypotheses && formData.hypotheses[0]) || '-'}</div>
                        <div class="label">H2 (Résistance)</div><div class="value">${(formData.hypotheses && formData.hypotheses[1]) || '-'}</div>
                        <div class="label">H3 (Fuite)</div><div class="value">${(formData.hypotheses && formData.hypotheses[2]) || '-'}</div>
                    </div>
                </div>
            </div>
        `;
            ...
            pages += `
                <div class="pdf-page"><h2>Articulation : ${block.type} - ${block.title || '-'}</h2><div class="grid">
                    ...
                    <div class="card"><h3>Composition</h3><div style="display: flex; gap: 5px; flex-wrap: wrap;">
                        ${(block.members || []).length > 0 ? block.members.map(m => `<span class="badge" style="background:#3b82f6; color:#fff;">${m}</span>`).join('') : '-'}
                    </div><div style="margin-top: 15px;"><span class="label">Place du Chef</span> ${block.place_chef || '-'}</div></div>
                </div></div>
            `;
        });
        ...
        // --- PAGE PATRACDVR ---
        pages += `
            <div class="pdf-page"><h2>7. RÉCAPITULATIF PATRACDVR</h2><div class="card"><table style="font-size: 0.7em;"><thead><tr><th style="width:60px;">VL</th><th style="width:50px;">PAX</th><th style="width:60px;">CELLULE</th><th style="width:100px;">FONCTION</th><th>PPALE</th><th>SEC.</th><th style="width:50px;">AFIS</th><th>EQPT/GREN.</th><th style="width:40px;">DIR</th></tr></thead><tbody>
                ${(formData.patracdvr_rows || []).length > 0 ? formData.patracdvr_rows.flatMap(row => (row.members.map((m, idx) => `<tr>
                ...
                </tr>`))).join('') : '<tr><td colspan="9">Aucun personnel renseigné.</td></tr>'}
            </tbody></table></div></div>
        `;
        ...
        pages += `<div class="pdf-page"><h2>8. CONDUITES À TENIR & LIAISON</h2><div class="grid">
            ...
            <div class="card"><h3>NO GO / DÉSOBSTRUCTION</h3><div class="value" style="color: ${colors.danger}; font-weight: bold;">${formData.no_go || '-'}</div></div>
        </div><div class="card"><h3>LIAISON & GESTUELLE</h3><div class="value">${formData.cat_liaison || '-'}</div></div>
